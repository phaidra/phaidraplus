...
function initializeMap() {
    ...
    //$("#ctlzoom").empty();
    //var zoomSlider = $("<input name='zoom' type='range'/>").attr("min",GexfJS.minZoom).attr("max",GexfJS.maxZoom).val(GexfJS.params.zoomLevel).attr("step",1).attr("id","zoomSlider");
    //$("#zoomSlider").on("change",function(e){

    //    GexfJS.params.zoomLevel = $(this).val();
    //})
    //zoomSlider.css("margin-top","20px");
    //$("#ctlzoom").append($("<label for='zoom'>Zoom</label>"))
    //$("#ctlzoom").append(zoomSlider);
    GexfJS.graphZone.width = $(window).width();//_elZC.width();
    GexfJS.graphZone.height = $(window).height();//_elZC.height();
    $(".carte")
        .attr({
